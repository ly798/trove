...
class SwiftStorage(base.Storage):
    ...
    def save(self, filename, stream, metadata=None):
        ...
        LOG.info(_('Saving %(filename)s to %(container)s in swift.')
                 % {'filename': filename, 'container': self.backup_container})

        # Create the container if it doesn't already exist
        LOG.debug('Creating container %s.' % self.backup_container)
        ...
        if large_object:
            ...
            self.connection.delete_object(self.backup_container,
                                          stream_reader.first_segment)
